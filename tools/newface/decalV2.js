...
const mergeTable = async () => {
    ...
    // 读取table
    const settingsTablePath = path.join(__dirname, `../../raw/std/settings/faceliftv2/decal.tab`);
    ...
    logger.info("读取 ui 表");
    const uiTablePath = path.join(__dirname, `../../raw/std/ui/faceliftv2/facedecalsv2.tab`);
...
const buildData = async () => {
    ...
    logger.info(`开始构建 v2 的 decal 数据`);
    const mergedTables = await mergeTable();
    ...
    logger.info(`开始写入文件`);
    const outputPath = path.join(__dirname, `../../assets/data/face/decal_v2.json`);

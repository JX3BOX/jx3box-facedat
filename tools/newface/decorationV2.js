...
const mergeTable = async () => {
    ...
    const uiTablePath = path.join(__dirname, `../../raw/std/ui/faceliftv2/decorationv2.tab`);
    ...
    const settingsTablePath = path.join(__dirname, `../../raw/std/settings/faceliftv2/decoration.tab`);
    ...
    const dataTablePath = path.join(__dirname, `../../raw/std/public/face/facepart_hd.tab`);
...
const buildData = async (client) => {
    ...
    logger.info(`开始写入文件`);
    const outputPath = path.join(__dirname, `../../assets/data/newface/decoration_v2.json`);
    await writeJSON(outputPath, ret);
    logger.info(`写入文件 ${outputPath} 成功`);
};

const main = async () => {
    baseLogger = initLogger('jx3-facedat/newface/decoration');

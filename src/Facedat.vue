                        ...
                        <ul class="u-list u-new" v-for="(subItem, itemIndex) in Object.keys(new_face_dict['轮廓'][item])" :key="itemIndex">
                            <template v-if="subItem === 'root'">
                                <li v-for="(key, i) in new_face_dict['轮廓'][item].root" :key="key + i">
                                    ...
                                    <slider v-if="lock" class="u-range" :min="-128" :max="128" :value="facedata['tBone'][key.BoneType]"></slider>
                                    <el-slider
                                        v-else
                                        class="u-range"
            ...
            <div class="c-facedat-group" id="decals" v-show="active === 'decals'">
                <el-tabs v-model="subActive.decals" :type="tab_type">
                    <el-tab-pane :label="item" :name="item" v-for="(item, index) in Object.keys(new_decal_group['妆容'])" :key="index">
                        <div v-for="(subItem, itemIndex) in new_decal_group['妆容'][item]" :key="itemIndex">
                            <ul class="u-decals">
                                <li v-if="facedata.tDecal[subItem.DecalsType].bUse">
                                    <div class="u-title">
                                        <img class="u-pic" :src="decalDb.getDecalIcon(subItem.DecalsType, facedata.tDecal[subItem.DecalsType]['nShowID'], true)" />
                                        {{ decalDb.getDecalName(subItem.DecalsType, facedata.tDecal[subItem.DecalsType]["nShowID"], true) }}
                                    </div>
                                    <span class="u-dname">
                                    ...
                                    <span class="u-price">
                                        <template v-if="decalDb.getDecalPrice(subItem.DecalsType, facedata.tDecal[subItem.DecalsType]['nShowID'], true)">
                                            <i class="el-icon-coin"></i>
                                            {{ decalDb.getDecalPrice(subItem.DecalsType, facedata.tDecal[subItem.DecalsType]["nShowID"], true) }}
                                            通宝</template
                                        >
                                    </span>
                                </li>
                    ...
                    <template v-if="cleandata['tDecal'][key]">
                        <ul class="u-decals">
                            <li v-show="!clean || checkdecal_prop(key)">
                                ...
                                >
                                <span class="u-dflip" v-if="decalDb.getDecalIsFlip(key, cleandata['tDecal'][key]['nShowID'])">(翻转)</span>
                                ...
                                <span class="u-price">
                                    <template v-if="decalDb.getDecalPrice(key, cleandata['tDecal'][key]['nShowID'])">
                                        <i class="el-icon-coin"></i>
                ...
                <div class="c-facedat-group">
                    <ul class="u-decals">
                        <li>
                            ...
                            <span class="u-dname"><img class="u-pic" :src="decalDb.getDecorationIcon(cleandata['nDecorationID'])" />{{ decalDb.getDecorationName(cleandata["nDecorationID"]) }}</span>
